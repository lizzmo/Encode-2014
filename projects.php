...
<?php get_header(); ?>
	...
	<div class="carousel">
		<?php // get children
		$current = $post->ID;
		$child_posts = get_children('post_parent='.$current.'&numberposts=-1&orderby=menu_order&order=ASC');
		if (!empty($child_posts)) {
			foreach ($child_posts as $post) {
				$postTitle = $post->post_title;
				$postID = $post->ID; 
				$noBrowser = get_post_meta($postID, 'browser', true); // check for browser bar
				$video = get_post_meta($postID, 'video', true); // check for video
				$images = get_posts('post_type=attachment&post_mime_type=image&post_parent='.$postID.'&posts_per_page=-1&orderby=menu_order&order=ASC'); // check for images
				if (!empty($video)) { // show video ?>
					<div>
						<ul class="slides">
							<li>
								<iframe src="http://player.vimeo.com/video/<?php echo $video; ?>?portrait=0&loop=0" width="100%" height="375" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>	
							</li>
						</ul>
					</div>
				<?php } else if (!empty($images)) { // if no video, show image ?>
					<div>
						<ul class="slides">
 ...
 height="16" viewBox="0 0 12 12" style="enable-background:new 0 0 12 12;" xml:space="preserve">
		...
		<div class="credits">
			...
			if (!empty($design)) { ?>
				<p>Design by <?php if (!empty($designURL)) { echo '<a class="animated" href="http://'.$designURL.'" target="_blank">'; } echo $design; if (!empty($designURL)) { echo '</a>'; } ?></p>
			<?php } else if (empty($design)) { ?>
				<p>Design & development<br />by Encode</p>
			<?php } ?>
		</div>
	</div>
<?php get_footer(); ?>