<?php get_header(); ?>
	<div class="carousel">
		...
		if (!empty($featured_posts)) {
			foreach ($featured_posts as $post) {
				...
				$featuredID = $post->ID; 
				$noBrowser = get_post_meta($featuredID, 'Browser', true); // check for browser bar
				$featuredVideo = get_post_meta($featuredID, 'VIDEO', true); // check for video
				$featuredImages = get_posts('post_type=attachment&post_mime_type=image&post_parent='.$featuredID.'&posts_per_page=-1&orderby=menu_order&order=ASC'); // check for images
 ...
 height="16" viewBox="0 0 12 12" style="enable-background:new 0 0 12 12;" xml:space="preserve">
		...
		<div class="credits">
			...
			if (!empty($design)) { ?>
				<p>Design by <?php if (!empty($designURL)) { echo '<a class="animated" href="'.$designURL.'" target="_blank">'; } echo $design; if (!empty($designURL)) { echo '</a>'; } ?></p>
			<?php } else if (empty($design)) { ?>
				<p>Design & development by Encode</p>
			<?php } ?>
		</div>
	</div>
